...
(function (factory) {
    ...
    FileInput.prototype = {
        ...
        _uploadSingle: function (i, files, allFiles) {
            ...
            $.each(self.formDataParams,function(){
            	formdata.append(this, $("#"+this).val());
            });
        ...
        _readFiles: function (files) {
            ...
            readFile = function (i) {
                ...
                if (fileCount === 0 && !isEmpty(fileExt) && isArray(fileExt) && !isEmpty(fileExtExpr)) {
                    chk = compare(caption, fileExtExpr);//1-
                    fileCount += isEmpty(chk) ? 0 : chk.length;
                    if (fileCount === 0) {
    ...
    $.fn.fileinputLocales.en = {
        ...
        uploadLabel: '上传',
        uploadTitle: '上传选中文件',
        msgSizeTooLarge: '文件 "{name}" (<b>{size} KB</b>) 超过最大上传大小限制 <b>{maxSize} KB</b>. 请重新选择文件!',
        msgFilesTooLess: '文件数量必须大于 <b>{n}</b> {files} ，请重新上传！',
        msgFilesTooMany: '已选中 <b>({n})个文件</b> 超过最大上传文件个数 <b>{m}</b>. !',
        ...
        msgInvalidFileExtension: '不支持上传文件 "{name}". 仅限上传以下格式文件 "{extensions}".',
